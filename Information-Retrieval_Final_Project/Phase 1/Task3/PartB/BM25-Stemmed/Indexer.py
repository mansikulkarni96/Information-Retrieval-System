    ...
    try:
        if(len(sorted_doc_score)>0):
            ...
            for i in range(min(100,len(sorted_doc_score))):
                ...
                out_file.write(str(QUERY_ID) + " Q0 "+ DOC_NAME[doc_id] +" " + str(i+1) + " " + str(doc_score) +" BM25_Model_Stemmed\n")            
            out_file.close()
...
def generate_doc_bm25_score(query,inverted_index,total_num_of_docs,relevant_list):
    try:
        ...
        for term in query_term_list:
            if not dict_query_tf.has_key(term):
                dict_query_tf.update({term:1})
            else:
                dict_query_tf[term]+=1
        for term in dict_query_tf:
            if inverted_index.has_key(term):
                reduced_inverted_index.update({term:inverted_index[term]})
            else:
                reduced_inverted_index.update({term:{}})
        process_score(dict_query_tf,reduced_inverted_index,total_num_of_docs,relevant_list)
    except Exception as e:
        print(traceback.format_exc())
...
def start():
    try:
        ...
        AVDL = generate_avdl()
        if exists(OUTPUT_FOLDER_PATH+"\\BM25_stemmed_doc_score.txt"):
            os.remove(OUTPUT_FOLDER_PATH+"\\BM25_stemmed_doc_score.txt")
