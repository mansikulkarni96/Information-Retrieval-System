...
def query_likelihood(query_term_freq,reduced_inverted_index,total_num_of_docs,words_per_doc):
    ...
    write_to_file(sorted_doc_scores)

def write_to_file(sorted_doc_score):
    try:
        if(len(sorted_doc_score)>0):
            out_file  = open(OUTPUT_FOLDER_PATH+"\\SQL_stopped_doc_score.txt",'a')
            for i in range(min(100,len(sorted_doc_score))):
                doc_id,doc_score = sorted_doc_score[i]
                out_file.write(str(QUERY_ID) + " Q0 "+ DOC_NAME[doc_id] +" " + str(i+1) + " " + str(doc_score) +" Smoothed_Query_Likelihood_Model_Stopped\n")            
            out_file.close()
...
def start():
    try:
        ...
        if exists(OUTPUT_FOLDER_PATH+"\\SQL_stopped_doc_score.txt"):
            os.remove(OUTPUT_FOLDER_PATH+"\\SQL_stopped_doc_score.txt")
        ...
        print "\nSQL Score Generated. Have a great day!"
    except Exception as e:
        print(traceback.format_exc())
