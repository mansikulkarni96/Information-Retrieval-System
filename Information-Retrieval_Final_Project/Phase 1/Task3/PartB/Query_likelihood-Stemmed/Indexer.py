...
def write_doc_score(sorted_doc_score):
    try:
        if(len(sorted_doc_score)>0):
            out_file  = open(OUTPUT_FOLDER_PATH+"\\Stemmed_QL_doc_score.txt",'a')
            for i in range(min(100,len(sorted_doc_score))):
                doc_id,doc_score = sorted_doc_score[i]
                out_file.write(str(QUERY_ID) + " Q0 "+ DOC_NAME[doc_id] +" " + str(i+1) + " " + str(doc_score) +" Stemmed_Query_Likelihood_Model\n")            
            out_file.close()
...
def start():
    ...
    if exists(OUTPUT_FOLDER_PATH+"\\Stemmed_QL_doc_score.txt"):
        os.remove(OUTPUT_FOLDER_PATH+"\\Stemmed_QL_doc_score.txt")
