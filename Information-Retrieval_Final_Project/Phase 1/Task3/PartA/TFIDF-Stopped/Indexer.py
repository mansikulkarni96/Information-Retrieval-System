...
def generate_index():
    ...
    try:
        ...
        for file in glob(os.path.join(INPUT_FOLDER,'*.txt')):
            ...
            with open('common_words') as cwords:
                stopwords = cwords.read().splitlines()
...
def write_to_file(sorted_doc_score):
    try:
        if(len(sorted_doc_score)>0):
            out_file  = open(OUTPUT_FOLDER_PATH+"\\TFIDF_stopped_doc_score.txt",'a')
            for i in range(min(100,len(sorted_doc_score))):
                doc_id,doc_score = sorted_doc_score[i]
                out_file.write(str(QUERY_ID) + " Q0 "+ DOC_NAME[doc_id] +" " + str(i+1) + " " + str(doc_score) +" tf_idf_Model_Stopped\n")
            out_file.close()
...
def start():
    try:
        ...
        if exists(OUTPUT_FOLDER_PATH+"\\TFIDF_stopped_doc_score.txt"):
            os.remove(OUTPUT_FOLDER_PATH+"\\TFIDF_stopped_doc_score.txt")
        query_file = open("query.txt", 'r')
        for query in query_file.readlines():
            QUERY_ID+=1
            generate_doc_tfidf_score(query,inverted_index,total_num_of_docs)
        print "\nTFIDF Score Generated. Have a great day!"
    except Exception as e:
        print(traceback.format_exc())
